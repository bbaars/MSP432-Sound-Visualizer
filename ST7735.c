...
#define TFT_CS                  (*((volatile uint8_t *)0x40004C42))    /* Port 5 Output, bit 0 is TFT CS */
//#define TFT_CS                  (*((volatile uint8_t *)0x40004C82))     /* Port 9 Output, bit 4 is TFT CS */
#define TFT_CS_BIT              0x00                                     // Bit 0 of Port 5
//#define TFT_CS_BIT              0x10                                    // CS normally controlled by hardware
#define DC                      (*((volatile uint8_t *)0x40004C42))   /* Port 5 Output, bit 2 is DC */
//#define DC                      (*((volatile uint8_t *)0x40004C82))     /* Port 9 Output, bit 2 is DC */
#define DC_BIT                  0x04                                    // Bit 2 of Port 5
#define RESET                    (*((volatile uint8_t *)0x40004C22))
...
uint8_t static writecommand(uint8_t c) {
  while((EUSCI_B0->IFG&0x0002)==0x0000){};    // wait until UCA3TXBUF empty
  DC &= ~DC_BIT;
  EUSCI_B0->TXBUF = c;                        // command out
  while((EUSCI_B0->IFG&0x0001)==0x0000){};    // wait until UCA3RXBUF full
...
void static commonInit(const uint8_t *cmdList) {
  ...
  P3->DIR |= BIT6;                      // Configure P3.6 (RST) As Output
  P5->DIR |= BIT0 | BIT2;               // Configure P5.0 (TFT_CS) & P5.2 (DC) As Output
